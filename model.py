...
#                     InpImages = InpImages.to( self.device)
#                     ROImap = ROImap.to( self.device)
# # -------------------------Normalize image -------------------------------------------------------------------------------------------------------
...
#
                ...
                AttentionMap = self.Valve[nValve](F.interpolate(ROI, size=x.shape[2:4], mode='bilinear'))#(F.upsample(ROImap, size=x.shape[2:4], mode='bilinear'))
                x = x + AttentionMap
                nValve += 1
                # ---------------------First resnet block-----------------------------------------------------------------------------------------------
                ...
                return x
# model = Net(NumClasses=1,UseGPU=True)
# print(model)